...
public class QatInputStream extends FilterInputStream {
  ...
  /**
   * Creates a new input stream with the given algorithm, {@link QatZipper#DEFAULT_COMPRESS_LEVEL},
   * and {@link Mode#AUTO}.
   *
   * @param in the input stream
   * @param bufferSize the input buffer size
   ...
   */
  public QatInputStream(InputStream in, int bufferSize, Algorithm algorithm, int level) {
   ...
   *     failover.)
   */
  public QatInputStream(InputStream in, int bufferSize, Algorithm algorithm, int level, Mode mode) {
    super(in);
